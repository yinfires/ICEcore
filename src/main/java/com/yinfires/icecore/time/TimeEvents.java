...
public final class TimeEvents {
    private TimeEvents(){}
    @SubscribeEvent public static void started(ServerStartedEvent e){TimeConfigManager.get().start(e.getServer());TimeService.start(e.getServer());DewDropFarmlandTimeCompat.start(e.getServer());TimeVoteManager.start(e.getServer());}
    @SubscribeEvent public static void stopping(ServerStoppingEvent e){TimeVoteManager.stop();DewDropFarmlandTimeCompat.stop();TimeService.stop();TimeConfigManager.get().stop();}
    ...
    @SubscribeEvent public static void levelTick(TickEvent.LevelTickEvent e){if(e.level instanceof ServerLevel level){if(e.phase==TickEvent.Phase.START)DewDropFarmlandTimeCompat.beginLevelTick(level);else DewDropFarmlandTimeCompat.endLevelTick(level);}}
